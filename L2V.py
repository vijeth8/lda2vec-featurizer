...
class l2v:
    ...
    def initialize_test(self,\
        ...
        logging.warning('Test parameters initialized!')

    def test(self,docs=None,epochs=200, update_words=False, update_topics=False):
        
        self.preprocess(docs)
        
        logging.warning('preprocessed!')
        
        self.test_model = LDA2Vec(n_documents=self.n_docs,\
                        n_document_topics=self.n_topics,\
                        n_units=300,\
                        n_vocab=self.n_vocab,\
                        counts=self.term_frequency,\
        ...
        if self.words_pretrained:
            self.test_model.sampler.W.data[:, :] = self.vectors[:self.n_vocab, :]

        self.test_model.to_gpu()
        self.test_model.mixture.factors.W.data[:, :] = self.train_model.mixture.factors.W.data
        optimizer = O.Adam()
        optimizer.setup(self.test_model)
        clip = chainer.optimizer.GradientClipping(5.0)
        optimizer.add_hook(clip)
        ...
        for epoch in range(epochs):
            print "epoch : ",epoch
            data = prepare_topics(cuda.to_cpu(self.test_model.mixture.weights.W.data).copy(),
                                  cuda.to_cpu(self.test_model.mixture.factors.W.data).copy(),
                                  cuda.to_cpu(self.test_model.sampler.W.data).copy(),
                                  self.words)
            top_words = print_top_words_per_topic(data)
            if j % 100 == 0 and j > 100:
                coherence = topic_coherence(top_words)
            ...
            for d, f in utils.chunks(self.batchsize, self.doc_ids, self.flattened):
                t0 = time.time()
                optimizer.zero_grads()
                l = self.test_model.fit_partial(d.copy(), f.copy(), update_words=update_words, update_topics=update_topics)
                prior = self.test_model.prior()
                loss = prior * self.fraction
                loss.backward()
                optimizer.update()
                msg = ("J:{j:05d} E:{epoch:05d} L:{loss:1.3e} "
